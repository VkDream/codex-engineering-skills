...
- Start from the baseline route: current agent, one primary Skill when triggered, the narrowest directly relevant callable MCP, and no subagent. Call relevant MCP automatically; add a second MCP route only for a distinct evidence target, and run independent calls concurrently when safe.
- If the route starts a background or long-lived process, apply the global process-ownership gate; this router never owns or cleans a host MCP process.
- Use only as many subagents as there are useful independent work items. Follow the global `AGENTS.md` for the parallel ceiling; a lower current contract or runtime limit still applies. Do not fill spare capacity with artificial tasks. Model, reasoning, service-tier, Fast/Sol restrictions, pinned-role selection, and fallback behavior remain owned by the global `AGENTS.md`; this router only selects an allowed route and records the actual result. If the required non-Fast route is unavailable or cannot be confirmed, keep the work in the main task and record `SUBAGENT_TIER_BLOCKED` rather than silently falling back.
- Keep delegation one level deep, send only task-relevant context, and end the subagent after its result is integrated. A child must never spawn another child.
- Reduce context at the source before adding a compression layer: narrow the query, bound the result, and avoid rereading known material. Treat external compression packages as candidates until an isolated simulation preserves all critical markers on representative data. Do not enable a proxy, wrapper, MCP registration, persistent memory, or learning/writer feature without current explicit authorization.
- Batch small, independent, same-shape work that shares one verification surface into one subagent dispatch. Do not create one child per file, finding, test, or mechanical edit; keep work separate only when it needs different judgment, state ownership, permissions, or independent evidence.
- Hand off exact file pointers, stable contract identifiers, and compact evidence capsules instead of repeatedly pasting plans, logs, or diffs into prompts. A reviewer may reuse current, legible evidence while independently judging its relevance; rerun only when evidence is stale, missing, contradictory, affected by later changes, or the requested evidence level requires a fresh independent run.
- For enabled knowledge-base stage relay, read and execute `references/kb-stage-relay.md` instead of restating its state machine here. The router only enforces its entry conditions: the relay is user-owned rather than a subagent, uses one user-valued milestone and one master RUN, loads only the current fixed role's `kb-role-*` Skill, and keeps `03·审查` independent.
- Relay dispatch remains event-driven: after one exact dispatch stop; never poll or broadcast. Normal completion and hard stop use their single contract targets, stale/duplicate/unexpected callbacks cannot advance state, and continuation uses the reference's executable checkpoint/ACK protocol only after user confirmation.
